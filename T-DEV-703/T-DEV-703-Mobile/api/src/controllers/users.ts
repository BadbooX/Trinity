...
export async function updateUser(
  ...
  let data = request.params;
  const userId = Number(data.userId); // Assurez-vous que userId est un nombre
  if (isNaN(userId)) {
    return reply.code(400).send({ message: "Invalid user ID" });
  }
  ...
  try {
    ...
    if (updates.address && updates.address.id) {
      ...
      const address = await fastify.prisma.address.findFirst({
        where: { id: updates.address.id, UserId: userId },
      });
      if (!address) {}
      else {
    ...
    if (updates.address && !updates.address.id) {
      if (!updates.address.country 
        ...
        || updates.address.postalCode === undefined 
...
export async function modifyMyUser(
  ...
  updateUser(request as FastifyRequest<{ Params: { userId: number }, Body: UpdateUserData }>, reply, fastify);
}
